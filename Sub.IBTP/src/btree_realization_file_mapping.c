...
int masterLoad(struct KV* kv) {
	if (kv->mmap.total == 0) {
		...
		key* key = dump_pack(strlen("license"), "license", 1);
		value* val = dump_pack(strlen("Welcom to my database"), "Welcom to my database", 1);
		Insert(&kv->tree, key, val);
		dump_delete_fr(key);
		dump_delete_fr(val);
	}
	else
	{
		...
		unsigned char bad = !((1 <= used) && (used <= (kv->mmap.total / BTREE_PAGE_SIZE)));
...
int Del(struct KV* db, key* key) {
	unsigned char deleted = Delete(&db->tree, key);
	int err;
	if (err = flushPages(db)) return -1;
	else return (int)deleted;
}
